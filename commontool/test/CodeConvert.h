...
public:
    ...
    int Convert(char *src_buff,int str_len,char *dest_buff,int dest_len)
    {
        if (str_len == 0)
            return 0;
        int len = ::MultiByteToWideChar(CP_ACP, 0, src_buff, -1, NULL, 0);
        if (len <= 0)
            return 0;
        WCHAR* wch = new WCHAR[len];
        ::MultiByteToWideChar(CP_ACP, 0, src_buff, -1, wch, len);
        ...
        memcpy(dest_buff, ch, min(dest_len, len));
        delete [] ch;
        delete [] wch;
        return len;
    }
...
public:
    ...
    int Convert(char *src_buff,int str_len,char *dest_buff,int dest_len)
    {
        if (str_len == 0)
            return 0;
        int len = ::MultiByteToWideChar(CP_UTF8, 0, src_buff, -1, NULL, 0);
        if (len <= 0)
            return 0;
        WCHAR* wch = new WCHAR[len];
        ::MultiByteToWideChar(CP_UTF8, 0, src_buff, -1, wch, len);
...
public:
	...
	int Convert(char *szSrcBuf,int iSrcLen,char *szDestbuf,int* iDestlen)
	{
		...
		int len = *iDestlen;
		int ret = iconv(m_ConvertCode,pin,(size_t *)&iSrcLen,pout,(size_t *)iDestlen);
