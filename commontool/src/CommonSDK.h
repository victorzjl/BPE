...
interface ISDOCommonMsg
{
	...
	/* for ext head */
	SNDAMETHOD(void) SetExtHead(IN const void * pExtHead, IN int nLen) PURE;
	SNDAMETHOD(void) GetExtHead(OUT void ** pExtHead, OUT int * pLen) PURE;
...
{
	 SNDAMETHOD(int) Initialize(IN const char *pConfigFile, IN unsigned int dwServiceId, IN const RequestCallBack pReqCb, IN const ResponseCallBack pResCb) PURE;
	 SNDAMETHOD(void) Uninitialize() PURE;

	 SNDAMETHOD(unsigned int) GetRpcId()PURE;
	 SNDAMETHOD(int) ASyncSendRequest2(IN const ISDOCommonMsg* pRequest, OUT unsigned int dwRpcId) PURE;
