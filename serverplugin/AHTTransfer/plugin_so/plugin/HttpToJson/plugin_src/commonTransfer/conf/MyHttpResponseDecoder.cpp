...
int MyCHttpResponseDecoder::Decode(const char *pBuffer, int nLen)
{
	SO_XLOG(XLOG_DEBUG,"MY--MyCHttpResponseDecoder::%s::%d  nLen[%d]  [\n%s\n]\n",__FUNCTION__,__LINE__,nLen,pBuffer);
	...
	SO_XLOG(XLOG_DEBUG,"MyCHttpResponseDecoder::%s what ??? \n",__FUNCTION__);
    ...
    {   
		...
		toLower(pHead);
		SO_XLOG(XLOG_DEBUG,"getHeadValue pHead[%s]--pValue[%s]\n",pHead,pValue);
	...
	if(isChunked)
	{
		...
		{ 
			...
			{
				SO_XLOG(XLOG_WARNING,"MyCHttpResponseDecoder::%s http response illegal.   pBuffer[0X%0X], nLen[%d], pBegin[0X%0X],nLength[%d],pBodyEnd[0X%0X]\n",
	...
	else
	{
		...
		else
		{
			...
			{
				SO_XLOG(XLOG_WARNING,"MyCHttpResponseDecoder::%s http response illegal.   pBuffer[0X%0X], nLen[%d], pHeadEnd[0X%0X]\n",__FUNCTION__,pBuffer,nLen,pHeadEnd);
				SO_XLOG(XLOG_WARNING,"MyCHttpResponseDecoder::%s::%d  nLen[%d]  [\n%s\n]\n",__FUNCTION__,__LINE__,nLen,pBuffer);
				return -4;
			}
		}
	}
...
string MyCHttpResponseDecoder::GetHeadValue(const string &strHead)
{
	SO_XLOG(XLOG_DEBUG,"MY--MyCHttpResponseDecoder::%s\n",__FUNCTION__);
...
void MyCHttpResponseDecoder::Dump()
{
	SO_XLOG(XLOG_DEBUG,"=========MyCHttpResponseDecoder::DUMP()=======\n");
...
map<string,string> MyCHttpResponseDecoder::GetHeadValue()
{
	return m_mapHeadValue;
}
