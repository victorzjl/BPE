...
CServiceConfig::CServiceConfig():m_bIsTreeStruct(true)
{
	...
	m_mapEHttpDataFormatByName["json"] = HTTP_DATA_JSON;
	m_mapEHttpDataFormatByName["keyvalue"] = HTTP_DATA_KEYVALUE;
	m_mapEHttpDataFormatByName["value"] = HTTP_DATA_VALUE;
	m_mapEHttpDataFormatByName["body"] = HTTP_DATA_BODY;
}


int CServiceConfig::LoadConfigTypeConfig_(TiXmlElement * pConfigType)
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::LoadConfigTypeConfig_\n");
	SConfigType oConfigType;
	
	if(GetTypeAttr_(pConfigType, oConfigType) < 0)
	{
		HT_XLOG(XLOG_ERROR, "xml GetTypeAttr failed\n");
    	return -1;
	}
	...
	if(oConfigType.eType == MSG_FIELD_TLV || oConfigType.eType == MSG_FIELD_STRUCT)
	{
		...
		while(NULL!=(pConfigField =(TiXmlElement *) pConfigType->IterateChildren(pConfigField)))
		{
			...
			{
				HT_XLOG(XLOG_ERROR, "xml GetFieldAttr failed\n");
				return -1;
			}
...
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::LoadMsgField_\n");
	TiXmlElement * pConfigField = NULL;
	...
	while(pConfigField)
	{
		...
		if(itrMapByTypeName != m_mapTypeByName.end())
		{
			...
			if(oItemType.eType == MSG_FIELD_ARRAY)
			{
				...
				if(itrMapByTypeName != m_mapTypeByName.end())
				{
					...
					HT_XLOG(XLOG_TRACE, "set the array element code[%d] type[%s]\n", oItemElementType.nCode,
						oItemType.strName.c_str());
				}
				else 
				{
					HT_XLOG(XLOG_ERROR, "can not find the array element type[%s]\n", oItemType.strItemType.c_str());
    				return -1;
				}
			}
		}
		...
		{
			HT_XLOG(XLOG_ERROR, "can not find the type[%s]\n", oConfigField.strTypeName.c_str());
			return -1;
		}

		if (bRequest)
		{
			oConfigMessage.oRequestType.mapPosByName.insert(make_pair(oConfigField.strOriName, dwIndex++));
		}
		else
		{
			oConfigMessage.oResponseType.mapPosByName.insert(make_pair(oConfigField.strOriName, dwIndex++));
		}
...
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::LoadConfigMsgConfig_\n");
	...
	{
		HT_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ have no requestParameter node\n");
    	return -1;
	}
	if(LoadMsgField_(oConfigMessage, pConfigFields) != 0)
	{
		HT_XLOG(XLOG_ERROR, "Load Msg Request Field failed failed\n");
    	return -1;
	}
	...
	if(pConfigFields == NULL)
	{
		HT_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ have no responseparameter node\n");
    	return -1;
	}
	if(LoadMsgField_(oConfigMessage, pConfigFields, false) != 0)
	{
		HT_XLOG(XLOG_ERROR, "Load Msg Response Field failed\n");
    	return -1;
	}
...
int	CServiceConfig::LoadServiceConfig(const string &strConfig)
{
    ...
    {
       	HT_XLOG(XLOG_ERROR, "xml load file[%s] failed\n", strConfig.c_str());
        return -1;
    }
	
    if((pService=m_xmlDoc.RootElement())==NULL)
    {
        HT_XLOG(XLOG_ERROR, "xml no root node\n");
        return -1;
    }
	...
	{
		HT_XLOG(XLOG_ERROR, "serice node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "serice node get id attr failed\n");
    	return -1;
	}
	...
	{
		HT_XLOG(XLOG_ERROR, "xml have no type node\n");
		return -1; 
	}
	else
	{	
		...
		{
			HT_XLOG(XLOG_ERROR, "LoadConfigTypeConfig_ failed\n");
			return -1; 
		}
	}
	...
	{
		HT_XLOG(XLOG_ERROR, "xml have no message node\n");
		return -1; 
	}
	else
	{	
		...
		{
			HT_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ failed\n");
			return -1; 
		}
	}
...
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetTypeAttr\n");
	char szName[64] = {0};
	int nRet = pElement->QueryValueAttribute("name", &szName);
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "type node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "type node get class attr failed\n");
    	return -1;
	}
	...
	if(oConfigType.eType == MSG_FIELD_ARRAY)
	{
		...
		if(nRet != TIXML_SUCCESS)
		{
			HT_XLOG(XLOG_ERROR, "type node get itemType attr failed\n");
	    	return -1;
		}
	...
	{
		HT_XLOG(XLOG_ERROR, "type node get code attr failed\n");
    	return -1;
	}
	...
	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetTypeAttr name[%s] type[%d] code[%d] default[%d] \n",
...
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetFieldAttr\n");
	char szName[64] = {0};
	int nRet = pElement->QueryValueAttribute("name", &szName);
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "field node get name attr failed\n");
    	return -1;
	}
	oConfigField.hiddenLog = false;
	...
	nRet = pElement->QueryValueAttribute("hiddenlog", &szName);
	if(nRet == TIXML_SUCCESS)
	{
		if (strcmp(szName,"true")==0)
		{
			oConfigField.hiddenLog = true;
		}
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "field node get type attr failed\n");
    	return -1;
	}
	...
	else 
	{
		...
		if(nRet == TIXML_SUCCESS)
		{
			...
			nRet = GetTypeByName(oConfigField.strTypeName, objConfigType);
			if(nRet != 0)
			{
				HT_XLOG(XLOG_ERROR, "CServiceConfig::GetFieldAttr, TypeName[%s] not exist\n", oConfigField.strTypeName.c_str());
				return -1;
			}
			...
			{
				HT_XLOG(XLOG_ERROR, "CServiceConfig::GetFieldAttr, TypeName[%s] default not supported\n", oConfigField.strTypeName.c_str());
				return -1;
			}
		}
		else 
	...
	char szIsUrlParameter[16] = {0};
	nRet = pElement->QueryValueAttribute("isurlparameter", &szIsUrlParameter);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigField.bUrlParameter = false;
	}
	else 
	{
		string strIsUrlParameter = szIsUrlParameter;
		transform (strIsUrlParameter.begin(), strIsUrlParameter.end(), strIsUrlParameter.begin(), (int(*)(int))tolower);
		if(strcmp(strIsUrlParameter.c_str(), "true") == 0)
		{
			oConfigField.bUrlParameter = true;
		}
		else 
		{
			oConfigField.bUrlParameter = false;
	...
	char szSuffix[32] = {0};
	nRet = pElement->QueryValueAttribute("suffix", &szSuffix);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigField.strSuffix = "";
	}
	else
	{
		oConfigField.strSuffix = szSuffix;
	}

	char szIsReturnField[16] = {0};
	nRet = pElement->QueryValueAttribute("isreturnfield", &szIsReturnField);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigField.bReturnField = false;
	}
	else 
	{
		string strIsReturnField = szIsReturnField;
		transform (strIsReturnField.begin(), strIsReturnField.end(), strIsReturnField.begin(), (int(*)(int))tolower);
		if(strcmp(strIsReturnField.c_str(), "true") == 0)
		{
			oConfigField.bReturnField = true;
		}
		else 
		{
			oConfigField.bReturnField = false;
	...
	char szEncodeMode[16] = {0};
	nRet = pElement->QueryValueAttribute("encode", &szEncodeMode);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigField.eEncodeMode = ENCODE_UNKNOWN;
	}
	else 
	{
		string strEncodeMode = szEncodeMode;
		transform (strEncodeMode.begin(), strEncodeMode.end(), strEncodeMode.begin(), (int(*)(int))tolower);
		if(strcmp(strEncodeMode.c_str(), "gbk") == 0)
		{
			oConfigField.eEncodeMode = ENCODE_GBK;
		}
		else if(strcmp(strEncodeMode.c_str(), "utf8") == 0)
		{
			oConfigField.eEncodeMode = ENCODE_UTF8;
		}
		else
		{
			oConfigField.eEncodeMode = ENCODE_UNKNOWN;
	...
	char szJsonPath[64] = {0};
	nRet = pElement->QueryValueAttribute("jsonpath", &szJsonPath);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigField.strJsonPath = oConfigField.strOriName;
	}
	else 
	{
		oConfigField.strJsonPath = szJsonPath;
	}

	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetFieldAttr name[%s] type[%s] \n",
...
{
	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetMsgAttr_\n");
	...
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "message node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		HT_XLOG(XLOG_ERROR, "message node get id attr failed\n");
    	return -1;
	}
	//获取url
	char szUrl[64] = {0};
	nRet = pElement->QueryValueAttribute("url", &szUrl);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigMessage.url = "";
	}
	...
	{
		oConfigMessage.url=szUrl;
	}
	//获取发送函数
	char szSendFunc[64] = {0};
	nRet = pElement->QueryValueAttribute("SendFunc", &szSendFunc);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigMessage.SendFunc = "";
	}
	else
	{
		oConfigMessage.SendFunc = szSendFunc;		
	}
	//获取接收函数
	char szReceiveFunc[64] = {0};
	nRet = pElement->QueryValueAttribute("ReceiveFunc", &szReceiveFunc);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigMessage.ReceiveFunc = "";
	}
	else
	{
		oConfigMessage.ReceiveFunc = szReceiveFunc;		
	}
	//获取发送方法
	char szMethod[64] = {0};
	nRet = pElement->QueryValueAttribute("HttpMethod", &szMethod);
	if(nRet != TIXML_SUCCESS)
	{
		oConfigMessage.method = "";
	}
	else
	{
		oConfigMessage.method = szMethod;		
	}
	
	

	HT_XLOG(XLOG_TRACE, "CServiceConfig::GetMsgAttr_ name[%s] id[%d] url[%s] SendFunc[%s] ReceiveFunc[%s] HttpMethod[%s]\n",
		oConfigMessage.strName.c_str(), oConfigMessage.dwId, oConfigMessage.url.c_str(),oConfigMessage.ReceiveFunc.c_str(), oConfigMessage.method.c_str());
...
{
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeByName name[%s]\n", strName.c_str());
	...
	HT_XLOG(XLOG_ERROR, "CServiceConfig::GetTypeByName name[%s] service[%d] failed\n", strName.c_str(), m_dwServiceId);
	return -1;
}

int CServiceConfig::GetTypeByCode(int nCode, SConfigType& oConfigType)
{
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeByCode code[%d]\n", nCode);
	...
	HT_XLOG(XLOG_ERROR, "CServiceConfig::GetTypeByCode code[%d] service[%d] failed\n", nCode, m_dwServiceId);
	return -1;
}
...
{
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetMessageTypeByName name[%s]\n", strName.c_str());
	...
	HT_XLOG(XLOG_ERROR, "CServiceConfig::GetMessageTypeByName name[%s] service[%d] failed\n", strName.c_str(), m_dwServiceId);
	return -1;
}
int CServiceConfig::GetMessageTypeById(unsigned int dwMessageId, SConfigMessage ** oMessageType)
{
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetMessageTypeById code[%d]\n", dwMessageId);
	...
	HT_XLOG(XLOG_ERROR, "CServiceConfig::GetMessageTypeById id[%d] service[%d] failed\n", dwMessageId, m_dwServiceId);
	return -1;
}
...
{
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeNameByFieldName PreviousTypeName[%s] fieldname[%s] len[%d]\n", 
	...
	{
		HT_XLOG(XLOG_ERROR, "get ParentType failed\n");
		return -1;
	}
	...
	HT_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeNameByFieldName ParentTypeName[%s]\n", strParentTypeName.c_str());
...
{
	HT_XLOG(XLOG_NOTICE, "############begin dump############\n");

	HT_XLOG(XLOG_NOTICE, "mapTypeByName,size[%d]\n",m_mapTypeByName.size());
	HT_XLOG(XLOG_NOTICE, "mapTypeByCode,size[%d]\n",m_mapTypeByCode.size());
	...
	for(itrType = m_mapTypeByName.begin(); itrType != m_mapTypeByName.end(); itrType++)
	{
		...
		HT_XLOG(XLOG_DEBUG, "name[%s] code[%d] type[%d] itemType[%s] fieldSize[%d]\n", 
	...
	HT_XLOG(XLOG_NOTICE, "mapMessageByName,size[%d]\n",m_mapMessageByName.size());
	HT_XLOG(XLOG_NOTICE, "mapMessageById,size[%d]\n",m_mapMessageById.size());
	...
	for(itrMessage = m_mapMessageByName.begin(); itrMessage != m_mapMessageByName.end(); itrMessage++)
	{
		...
		HT_XLOG(XLOG_DEBUG, "name[%s] id[%d] request[%d] res[%d] array[%d]\n", 
	...
	HT_XLOG(XLOG_NOTICE, "mapTypeNameByFieldName,size[%d]\n",m_mapTypePairByFieldName.size());
	...
	for(itrMap = m_mapTypePairByFieldName.begin(); itrMap != m_mapTypePairByFieldName.end(); itrMap++)
	{
		...
		HT_XLOG(XLOG_DEBUG, "fieldname[%s] fieldlen[%d] typename[%s] pretypename[%s]\n", 
