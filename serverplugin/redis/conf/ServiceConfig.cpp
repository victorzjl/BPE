...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::LoadConfigTypeConfig_\n");
	SConfigType oConfigType;
	
	if(GetTypeAttr_(pConfigType, oConfigType) < 0)
	{
		RVS_XLOG(XLOG_ERROR, "xml GetTypeAttr failed\n");
    	return -1;
	}
	...
	if(oConfigType.eType == MSG_FIELD_TLV || oConfigType.eType == MSG_FIELD_STRUCT)
	{
		...
		while(NULL!=(pConfigField =(TiXmlElement *) pConfigType->IterateChildren(pConfigField)))
		{
			...
			{
				RVS_XLOG(XLOG_ERROR, "xml GetFieldAttr failed\n");
				return -1;
			}
...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::LoadMsgField_\n");
	TiXmlElement * pConfigField = NULL;
	...
	while(pConfigField)
	{
		...
		if(itrMapByTypeName != m_mapTypeByName.end())
		{
			...
			if(oItemType.eType == MSG_FIELD_ARRAY)
			{
				...
				if(itrMapByTypeName != m_mapTypeByName.end())
				{
					...
					RVS_XLOG(XLOG_TRACE, "set the array element code[%d] type[%s]\n", oItemElementType.nCode,
						oItemType.strName.c_str());
				}
				else 
				{
					RVS_XLOG(XLOG_ERROR, "can not find the array element type[%s]\n", oItemType.strItemType.c_str());
    				return -1;
				}
			}
		}
		...
		{
			RVS_XLOG(XLOG_ERROR, "can not find the type[%s]\n", oConfigField.strTypeName.c_str());
			return -1;
		}
...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::LoadConfigMsgConfig_\n");
	...
	{
		RVS_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ have no requestParameter node\n");
    	return -1;
	}
	if(LoadMsgField_(oConfigMessage, pConfigFields) != 0)
	{
		RVS_XLOG(XLOG_ERROR, "Load Msg Request Field failed failed\n");
    	return -1;
	}
	...
	if(pConfigFields == NULL)
	{
		RVS_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ have no responseparameter node\n");
    	return -1;
	}
	if(LoadMsgField_(oConfigMessage, pConfigFields, false) != 0)
	{
		RVS_XLOG(XLOG_ERROR, "Load Msg Response Field failed\n");
    	return -1;
	}
...
int	CServiceConfig::LoadServiceConfig(const string &strConfig)
{
    ...
    {
       	RVS_XLOG(XLOG_ERROR, "xml load file[%s] failed\n", strConfig.c_str());
        return -1;
    }
	
    if((pService=m_xmlDoc.RootElement())==NULL)
    {
        RVS_XLOG(XLOG_ERROR, "xml no root node\n");
        return -1;
    }
	...
	{
		RVS_XLOG(XLOG_ERROR, "serice node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "serice node get id attr failed\n");
    	return -1;
	}
	...
	nRet = pService->QueryValueAttribute("redisType", &szEncode);
	if(nRet != TIXML_SUCCESS)
	{
		m_eRedisType = RVS_REDIS_STRING;
	}else
	{
	    string strRedisType = szEncode;
		transform (strRedisType.begin(), strRedisType.end(), strRedisType.begin(), (int(*)(int))tolower);
		
		if(strcmp(strRedisType.c_str(), "set") == 0)
		{
			m_eRedisType = RVS_REDIS_SET;
		}
		else if(strcmp(strRedisType.c_str(), "zset") == 0)
		{
			m_eRedisType = RVS_REDIS_ZSET;
		}
		else if(strcmp(strRedisType.c_str(), "list") == 0)
		{
		    m_eRedisType = RVS_REDIS_LIST;
		}
		else if(strcmp(strRedisType.c_str(), "hashtable") == 0)
		{
			m_eRedisType = RVS_REDIS_HASH;
		}
		else
		{
		    m_eRedisType = RVS_REDIS_STRING;
	...
	{
		RVS_XLOG(XLOG_ERROR, "xml have no type node\n");
		return -1; 
	}
	else
	{	
		...
		{
			RVS_XLOG(XLOG_ERROR, "LoadConfigTypeConfig_ failed\n");
			return -1; 
		}
	}
    ...
    //对struct类型的field信息进行检查，field名字必须存在于type信息中
    map<int, SConfigType>::iterator configTypeIter;
	for(configTypeIter = m_mapTypeByCode.begin(); configTypeIter != m_mapTypeByCode.end(); configTypeIter++)
	{
		SConfigType &configType = configTypeIter->second;
		if(configType.eType == MSG_FIELD_STRUCT)
		{
			vector<SConfigField> &vecField = configType.vecConfigField;
			for(unsigned int i = 0; i < vecField.size(); i++)
			{
			    SConfigField &configField = vecField[i];
				map<string, SConfigType>::const_iterator iter = m_mapTypeByName.find(configField.strName);
				if(iter == m_mapTypeByName.end())
				{
					RVS_XLOG(XLOG_ERROR, "struct[%s] type check failed : field[%s] not exist in type message\n", configType.strName.c_str(), configField.strName.c_str());
					return -1;
				}
				else
				{
					configField.nCode = iter->second.nCode;
					if(configField.eStructFieldType != MSG_FIELD_INT && configField.nLen <= 0 && i < vecField.size()-1)
		            {
		                 RVS_XLOG(XLOG_ERROR, "struct[%s] type check failed : len of field[%s] is not set!\n", configType.strName.c_str(), configField.strName.c_str());
		    	         return -1;
		            }
					//RVS_XLOG(XLOG_DEBUG, "fieldName[%s] fieldCode[%d] fieldType[%d]\n", __FUNCTION__, configField.strName.c_str(), configField.nCode, configField.eStructFieldType);
				}
			}
		}
	}
	...
	{
		RVS_XLOG(XLOG_ERROR, "xml have no message node\n");
		return -1; 
	}
	else
	{	
		...
		{
			RVS_XLOG(XLOG_ERROR, "LoadConfigMsgConfig_ failed\n");
			return -1; 
		}
	}
...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetTypeAttr\n");
	char szName[64] = {0};
	int nRet = pElement->QueryValueAttribute("name", &szName);
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "type node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "type node get class attr failed\n");
    	return -1;
	}
	...
	if(oConfigType.eType == MSG_FIELD_ARRAY)
	{
		...
		if(nRet != TIXML_SUCCESS)
		{
			RVS_XLOG(XLOG_ERROR, "type node get itemType attr failed\n");
	    	return -1;
		}
	...
	{
		RVS_XLOG(XLOG_ERROR, "type node get code attr failed\n");
    	return -1;
	}
	...
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetTypeAttr name[%s] type[%d] code[%d] default[%d] \n",
...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetFieldAttr\n");
	char szName[64] = {0};
	int nRet = pElement->QueryValueAttribute("name", &szName);
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "field node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "field node get type attr failed\n");
    	return -1;
	}
	...
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetFieldAttr name[%s] type[%s] \n",
...
{
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetMsgAttr_\n");
	char szName[64] = {0};
	int nRet = pElement->QueryValueAttribute("name", &szName);
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "message node get name attr failed\n");
    	return -1;
	}
	...
	if(nRet != TIXML_SUCCESS)
	{
		RVS_XLOG(XLOG_ERROR, "message node get id attr failed\n");
    	return -1;
	}
	...
	RVS_XLOG(XLOG_TRACE, "CServiceConfig::GetMsgAttr_ name[%s] id[%d] bAck[%d]\n",
...
{
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeByName name[%s]\n", strName.c_str());
	...
	RVS_XLOG(XLOG_ERROR, "CServiceConfig::GetTypeByName name[%s] service[%d] failed\n", strName.c_str(), m_dwServiceId);
	return -1;
}

int CServiceConfig::GetTypeByCode(int nCode, SConfigType& oConfigType)
{
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeByCode code[%d]\n", nCode);
	...
	RVS_XLOG(XLOG_ERROR, "CServiceConfig::GetTypeByCode code[%d] service[%d] failed\n", nCode, m_dwServiceId);
	return -1;
}
...
{
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetMessageTypeByName name[%s]\n", strName.c_str());
	...
	RVS_XLOG(XLOG_ERROR, "CServiceConfig::GetMessageTypeByName name[%s] service[%d] failed\n", strName.c_str(), m_dwServiceId);
	return -1;
}
int CServiceConfig::GetMessageTypeById(unsigned int dwMessageId, SConfigMessage ** oMessageType)
{
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetMessageTypeById code[%d]\n", dwMessageId);
	...
	RVS_XLOG(XLOG_ERROR, "CServiceConfig::GetMessageTypeById id[%d] service[%d] failed\n", dwMessageId, m_dwServiceId);
	return -1;
}
...
{
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeNameByFieldName PreviousTypeName[%s] fieldname[%s] len[%d]\n", 
	...
	{
		RVS_XLOG(XLOG_ERROR, "get ParentType failed\n");
		return -1;
	}
	...
	RVS_XLOG(XLOG_DEBUG, "CServiceConfig::GetTypeNameByFieldName ParentTypeName[%s]\n", strParentTypeName.c_str());
...
{
	RVS_XLOG(XLOG_NOTICE, "############begin dump############\n");

	RVS_XLOG(XLOG_NOTICE, "mapTypeByName,size[%d]\n",m_mapTypeByName.size());
	RVS_XLOG(XLOG_NOTICE, "mapTypeByCode,size[%d]\n",m_mapTypeByCode.size());
	...
	for(itrType = m_mapTypeByName.begin(); itrType != m_mapTypeByName.end(); itrType++)
	{
		...
		RVS_XLOG(XLOG_DEBUG, "name[%s] code[%d] type[%d] itemType[%s] fieldSize[%d]\n", 
	...
	RVS_XLOG(XLOG_NOTICE, "mapMessageByName,size[%d]\n",m_mapMessageByName.size());
	RVS_XLOG(XLOG_NOTICE, "mapMessageById,size[%d]\n",m_mapMessageById.size());
	...
	for(itrMessage = m_mapMessageByName.begin(); itrMessage != m_mapMessageByName.end(); itrMessage++)
	{
		...
		RVS_XLOG(XLOG_DEBUG, "name[%s] id[%d] request[%d] res[%d] array[%d]\n", 
	...
	RVS_XLOG(XLOG_NOTICE, "mapTypeNameByFieldName,size[%d]\n",m_mapTypePairByFieldName.size());
	...
	for(itrMap = m_mapTypePairByFieldName.begin(); itrMap != m_mapTypePairByFieldName.end(); itrMap++)
	{
		...
		RVS_XLOG(XLOG_DEBUG, "fieldname[%s] fieldlen[%d] typename[%s] pretypename[%s]\n", 
