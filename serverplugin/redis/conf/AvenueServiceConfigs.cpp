...
{
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::LoadAvenueServiceConfigs [%s]\n", strServiceConfigPath.c_str());
	
	string realPath = strServiceConfigPath;
	bool isCacheDirectory=isDirectory(strServiceConfigPath+"/redis");
	if (isCacheDirectory)
	{
		realPath=strServiceConfigPath+"/redis";
	}
	
	CDirReader oDirReader("*.xml");
	if(!oDirReader.OpenDir(realPath.c_str()))
	{
		RVS_XLOG(XLOG_ERROR, "OpenDir Failed[%s]\n", realPath.c_str());
		return -1;
	}
	...
	{
		RVS_XLOG(XLOG_DEBUG, "the dir is empty or no file is unfiltered, dir=[%s]\n", realPath.c_str());
		return -1;
	}
}
...
{    
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::%s [%s]\n", __FUNCTION__,strConfigFile.c_str());
    SServiceConfig oServiceConfig;
...
{
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::GetServiceIdByName [%s]\n", strServiceName.c_str());
	...
	if(iter != m_mapServiceConfigByName.end())
	{
		...
		{
			RVS_XLOG(XLOG_ERROR, "GetMessageTypeByName[%s] Failed\n", szMsgName);
			return -1;
		}
	...
	{
		RVS_XLOG(XLOG_WARNING, "CAvenueServiceConfigs::GetServiceIdByName [%s] failed\n", strServiceName.c_str());
		return -1;
	}
}
...
{
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::GetServiceNameById serviceid[%d], msgid[%d]\n", dwServiceId, dwMsgId);
	...
	if(iter != m_mapServiceConfigById.end())
	{
		...
		{
			RVS_XLOG(XLOG_ERROR, "GetMessageTypeById[%d] Failed\n", dwMsgId);
			return -1;
		}
	...
	{
		RVS_XLOG(XLOG_WARNING, "CAvenueServiceConfigs::GetServiceNameById serviceid[%d] msgid[%d] failed\n", dwServiceId, dwMsgId);
		return -1;
	}
}
...
{
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::GetServiceByName [%s]\n", strServiceName.c_str());
	...
	{
		RVS_XLOG(XLOG_WARNING, "CAvenueServiceConfigs::GetServiceByName [%s] failed\n", strServiceName.c_str());
		return -1;
	}
}
int CAvenueServiceConfigs::GetServiceById(unsigned int dwServiceId, SServiceConfig ** oSeriveConfig)
{
	RVS_XLOG(XLOG_DEBUG, "CAvenueServiceConfigs::GetServiceById serviceid[%d]\n", dwServiceId);
	...
	{
		RVS_XLOG(XLOG_WARNING, "CAvenueServiceConfigs::GetServiceById [%d] failed\n", dwServiceId);
		return -1;
	}
...
void CAvenueServiceConfigs::Dump() const
{
	...
	RVS_XLOG(XLOG_NOTICE, "mapServiceConfigByName,size[%d]\n",m_mapServiceConfigByName.size());
	...
	for(itrService = m_mapServiceConfigByName.begin(); itrService != m_mapServiceConfigByName.end(); itrService++)
	{
		...
		RVS_XLOG(XLOG_DEBUG, "name[%s] id[%d] type[%d]\n", 
