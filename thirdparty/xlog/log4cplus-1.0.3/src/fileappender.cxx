...
//split the path to two part,dir and filename 
//eg : "c:\abc\test.txt"  ==>  "c:\abc", "test.txt"
inline char* SplitFileName(const char* szPath,char* szDir,char* szFileName)
{
	char szBuf[MAX_PATH_LEN+1];
	strncpy(szBuf,szPath,MAX_PATH_LEN);
	char *p = strrchr(szBuf,'\\');
    if(p==NULL) p=strrchr(szBuf,'/');
	if (p)
	{		
		if (szFileName)
...
inline bool ExistFile(const char* szFile)
{
	return access(szFile,0) != -1;
}

inline bool MakeDirectory(const char* szPath)
{
	char szParent[MAX_PATH_LEN+1];

	if (ExistFile(szPath))
		return true;

	SplitFileName(szPath,szParent,NULL);

	if (!ExistFile(szParent))
	{
		if (!MakeDirectory(szParent))
			return false;
	}
#ifdef WIN32
	return (mkdir(szPath) == 0 || GetLastError() == ERROR_ALREADY_EXISTS);
#else
	return (mkdir(szPath,S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP) == 0 
		|| errno == EEXIST);
#endif
}
...
file_remove (tstring const & src)
{
    ...
    ret= _wremove (src.c_str ());
#else
    ret= std::remove (LOG4CPLUS_TSTRING_TO_STRING (src).c_str ());
#endif
	return (ret==0) ? 0 : -1;
}
...
file_rename (tstring const & src, tstring const & target)
{
	int ret=0;
#if defined (UNICODE) && defined (WIN32)
	ret= _wrename (src.c_str (), target.c_str ()) == 0 ? 0 : -1;
#else
	ret= rename (LOG4CPLUS_TSTRING_TO_STRING (src).c_str (),
...
{
    char szBuf[MAX_PATH_LEN+1];
	strncpy(szBuf,filename.c_str(),MAX_PATH_LEN);
    char *p = strrchr(szBuf,'\\');
    if(p==NULL) p=strrchr(szBuf,'/');
    if(p!=NULL)
    {
