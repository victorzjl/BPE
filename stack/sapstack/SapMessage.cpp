...
void CSapEncoder::EndValue(unsigned short dwLen)
{
    if(dwLen!=0)
...
void CSapDecoder::DecodeBodyAsTLV()
{
    ...
    while(ptrLoc<(unsigned char *)pBody+nBodyLen)
    {
       ...
       // SS_XLOG(XLOG_DEBUG,"CSapDecodeMsg::%s,nLen[%d],factlen[%d],ptrLoc+nFactLen:[%d],(unsigned char *)pBody+nBodyLen:[%d]\n",
         //   __FUNCTION__,nLen,nFactLen,ptrLoc+nFactLen,(unsigned char *)pBody+nBodyLen);
        if(nLen==0||ptrLoc+nFactLen>(unsigned char *)pBody+nBodyLen)
        {
            break;
        }
