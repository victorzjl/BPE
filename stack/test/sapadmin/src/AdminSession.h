...
class CAdminSession:public CMsgThread,public CSapAgentCallback
{

	typedef void (CAdminSession::*AdminFunc)(SAdminMsg *);	
public:
	CAdminSession();
	void OnLoginServer(const string &strIp,int nPort,const string &strName,const string &strPass);
...
public:
	...
	virtual void OnReceiveMessage(int nId,const void *pBuffer, int nLen);
